...
export const useGetCareWorker = (id?: string) => {
    ...
    return useQuery({
        queryKey: QK.user.management.getCareWorker(id),
        queryFn: async () => {
            const response = await api.getCareWorker(id);
            return response;
        },
        enabled: !!token,
    })
...
export const useGetCareManagers = (props?: {
    ...
    const api = new UserManagementController(token);
    return useQuery({
        queryKey: [QK.user.management.getCareManagers, token, props?.search],
        queryFn: async () => {
            const response = await api.getCareManagers({
                search: props?.search,
            });
            return response;
        },
