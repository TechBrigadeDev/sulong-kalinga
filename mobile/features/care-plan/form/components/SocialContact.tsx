...
const DEFAULT_INTERVENTIONS: Omit<SocialContactIntervention, "id">[] = [
  { name: "Assist in attending Senior Citizens activities", minutes: "" },
  { name: "Assist in going to the Senior Citizens Day Center", minutes: "" },
  { name: "Encourage visits from family, friends, or caregivers to provide companionship and emotional support", minutes: "" },
  { name: "Arrange regular phone calls or other communication mediums with loved ones to maintain social connections", minutes: "" },
  { name: "Connect with support groups for individuals with chronic diseases", minutes: "" },
];
...
interface SocialContactProps {
  ...
  onChange: (data: Partial<SocialContactData>) => void;
}

export const SocialContact = ({ data, onChange }: SocialContactProps) => {
  const [customIntervention, setCustomIntervention] = useState("");

  const toggleIntervention = (intervention: SocialContactIntervention) => {
  ...
  return (
    <ScrollView>
      <YStack padding="$4" gap="$4">
        <Card elevate>
          <Card.Header padded>
            <Text size="$6" fontWeight="bold">Social Contact Interventions</Text>
          </Card.Header>
          <Card.Footer padded>
            <YStack gap="$4">
