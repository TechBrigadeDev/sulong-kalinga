...
const DEFAULT_INTERVENTIONS: Omit<OutdoorActivityIntervention, "id">[] = [
  { name: "Going outside for fresh air and sunlight", minutes: "" },
  { name: "Walking in parks and other areas in the community", minutes: "" },
  { name: "Encourage/Assist in spending time outdoors", minutes: "" },
  { name: "Gardening activities", minutes: "" },
];

export interface OutdoorActivityData {
  interventions: OutdoorActivityIntervention[];
}

interface OutdoorActivityProps {
  data: OutdoorActivityData;
  onChange: (data: Partial<OutdoorActivityData>) => void;
}

export const OutdoorActivity = ({ data, onChange }: OutdoorActivityProps) => {
  const [customIntervention, setCustomIntervention] = useState("");

  const toggleIntervention = (intervention: OutdoorActivityIntervention) => {
  ...
  return (
    <ScrollView>
      <YStack padding="$4" gap="$4">
        <Card elevate>
          <Card.Header padded>
            <Text size="$6" fontWeight="bold">Outdoor Activities Interventions</Text>
          </Card.Header>
          <Card.Footer padded>
            <YStack gap="$4">
              ...
              <YStack gap="$2">
                <XStack gap="$2">
                  <Input
                    flex={1}
                    placeholder="Enter custom outdoor activity"
