...
class MedicationSchedulingController {
    ...
    async getSchedules(params?: { search?: string; page?: number; limit?: number }) {
        try {
            const response = await this.api.get("/medication-schedules", {
                params: {
            ...
            const validate = await medicationSchedulesResponseSchema.safeParseAsync(data);
            if (!validate.success) {
...
export const medicationSchedulingController = new MedicationSchedulingController();
