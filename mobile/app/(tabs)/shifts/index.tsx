        ...
        <YStack>
            ...
            <XStack style={{ gap: 8 }}>
                <Button
                    style={[
                        buttonBaseStyle,
                        (isArrive === 'arrived' || isArrive === 'departed') && disabledButtonStyle
                    ]}
                    onPress={onArrive}
                    disabled={isArrive === 'arrived' || isArrive === 'departed'}
                >
                    Arrived
                </Button>
                ...
                <Button
                    style={[
                        buttonBaseStyle,
                        (!isArrive || isArrive === 'departed') && disabledButtonStyle
                    ]}
                    onPress={onDepart}
                    disabled={!isArrive || isArrive === 'departed'}
                >
                    Departed
                </Button>
            </XStack>
...
const Screen = () => {
    const [currentShiftFetched, setCurrentShiftFetched] = useState([]);
    ...
    const fetchCurrentShifts = async () => {
        try {
            ...
            const data = await response.json();
            console.log("current shifts fetched", data)
    ...
    useEffect(() => {
        const intervalId = setInterval(fetchShifts, 20000); // Fetch every 20 seconds
    ...
    const ArrivedShift = async ({
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/shifts/${shift_id}/tracks/event`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/json",
            ...
            if (!response.ok) {
                ...
                let errorMessage = errorBody.message || `Error ${response.status}`;

                if (errorBody.errors && typeof errorBody.errors === 'object') {
                    const firstField = Object.keys(errorBody.errors)[0];
                    errorMessage = errorBody.errors[firstField]?.[0] || errorMessage;
                }
    ...
    const timeInShift = async () => {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (
                    response.status === 409 &&
                    json?.message === "Care worker already has an in-progress shift."
                ) {
                    console.log("⚠️ Already clocked in. Restoring shift state...");
                    await secureStorage.setItem("isOnShift", "true");
                    await secureStorage.setItem("currentShift", JSON.stringify(data));
    ...
    const timeOutShift = async ({
        ...
        showToastable({
            title: "Success",
            status: "success",
            message: "Shift timed out successfully.",
        });
        fetchShifts();
        console.log("Shift timeout success:", data);
        return data;
    ...
    const useShiftEvent = (
        ...
        const isOnShift = useShiftStore((state) => state.isOnShift);
        const currentShift = useShiftStore((state) => state.currentShift);
        const setOnShift = useShiftStore((state) => state.setOnShift);
        ...
        useEffect(() => {
            ...
            if (currentShiftFetched?.message === "No in-progress shift found.") {
                setOnShift(false);
                setCurrentShift(null);
            } else {
                setOnShift(true);
                setCurrentShift(currentShiftFetched);
            }
        }, [currentShiftFetched]);

        const shiftEvent = async (event: boolean) => {
            try {
                console.log("Current shift:", currentShift);
                console.log("Event (true=time-in, false=time-out):", event);

                if (event) {
                    const result = await timeInShift(location);
                    if (result) {
                        setOnShift(true);
                    ...
                    const result = await timeOutShift({
                        shift_id: currentShift?.shift?.id,
                        care_worker_id: currentShift?.shift?.care_worker_id,
                    });
    ...
    const { isOnShift, currentShift, shiftEvent } = useShiftEvent(location, currentShiftFetched);
                ...
                <YStack>
                    {shifts && shifts.length > 0 ? (
                        <>
                            ...
                            {shifts.map((item) => (
                                <VisitationCard
                                    key={item.visitation_id}
                                    name={item.beneficiary_name}
                                    address={item.address}
                                    isArrive={item?.current_status}
                                    time={
                                        item.start_time
                                            ? new Date(item.start_time).toLocaleString('en-PH', {
                                                weekday: 'long',
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric',
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                                { text: 'Cancel', style: 'cancel' },
                                                {
                                                    ...
                                                    onPress: () => {
                                                        if (
                                                            currentShift?.shift?.care_worker_id &&
                                                            currentShift?.shift?.id &&
                                                            item?.visitation_id
                                                        ) {
                                                            const processArrival = (loc) => {
                                                                if (loc) {
                                                                    ArrivedShift({
                                                                        care_worker_id: currentShift.shift.care_worker_id,
                                                                    ...
                                                                    showToastable({
                                                                        ...
                                                                        status: 'danger',
                                                                        message: 'Location must be granted.',
                                                                    });
                                                                }
                                                            };
                                                            if (!location) {
                                                                getCurrentLocation().then(processArrival);
                                                            } else {
                                                            ...
                                                            showToastable({
                                                                ...
                                                                status: 'danger',
                                                                message: 'Your shift is not started yet.',
                                                            });
                                                        }
                                                    },
                                                },
                                                ...
                                                { text: 'Cancel', style: 'cancel' },
                                                {
                                                    ...
                                                    onPress: () => {
                                                        if (
                                                            ...
                                                            if (!location) {
                                                                getCurrentLocation().then(processDeparture);
                                                            } else {
                                                                processDeparture(location);
                                                            }
                                                        } else {
                                                            showToastable({
                                                                title: 'Start your shift first.',
                        ...
                        >
                            ...
                            <Text style={{ fontSize: 14, color: '#666', textAlign: 'center' }}>
                                You currently have no scheduled visitations. Please check back later.
                            </Text>
                        </YStack>
                    )}
                </YStack>
