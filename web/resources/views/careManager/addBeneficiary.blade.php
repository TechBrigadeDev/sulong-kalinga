...
<head>
    ...
    <title>Add Beneficiary | Manager</title>
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="row" id="addUserForm">
                <div class="col-12">
                    ...
                    @endif
                    <form action="{{ route('care-manager.beneficiaries.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-3">
                                <label for="mobileNumber" class="form-label">{{ T::translate('Mobile Number', 'Numero ng Mobile')}}</label>
                                <div class="input-group">
                                    <span class="input-group-text">+63</span>
                                    <input type="text" class="form-control" id="mobileNumber"  name="mobile_number" value="{{ old('mobile_number') }}" placeholder="{{ T::translate('Enter mobile number', 'Ilagay ang mobile number')}}" maxlength="11" required oninput="restrictToNumbers(this)" title="{{ T::translate('Must be 10 or 11digits.', 'Dapat 10 o 11 digits.')}}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <label for="landlineNumber" class="form-label">{{ T::translate('Landline Number', 'Numero ng Landline')}}</label>
                                <input type="text" class="form-control" id="landlineNumber" name="landline_number" value="{{ old('landline_number') }}" placeholder="{{ T::translate('Enter Landline number', 'Ilagay ang Landline number')}}" maxlength="10" oninput="restrictToNumbers(this)" title="{{ T::translate('Must be between 7 and 10 digits.', 'Dapat nasa pagitan ng 7 at 10 digits.')}}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3 position-relative">
                                <label for="primaryCaregiver" class="form-label">{{ T::translate('Primary Caregiver', 'Pangunahing Tagapag-alaga')}}</label>
                                <input type="text" class="form-control" id="primaryCaregiver" name="primary_caregiver" value="{{ old('primary_caregiver') }}" placeholder="{{ T::translate('Enter Primary Caregiver name', 'Ilagay ang pangalan ng Pangunahing Tagapag-alaga')}}">                
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="addressDetails" class="form-label">{{ T::translate('House No., Street, Subdivision, Barangay, City, Province', 'Numero ng Bahay, Kalye, Subdivision, Barangay, Lungsod, Probinsya')}}<label style="color:red;"> * </label></label>
                                ...
                                required 
                                pattern="^[a-zA-Z0-9\s,.-]+$" 
                                title="{{ T::translate('Only alphanumeric characters, spaces, commas, periods, and hyphens are allowed.', 'Tanging mga alphanumeric na karakter, espasyo, kuwit, tuldok, at gitlang lamang ang pinapayagan.')}}"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <label for="searchAddress" class="form-label">{{ T::translate('Or search address', 'O maghanap ng address')}}</label>
                        ...
                        <div class="row mb-1">
                            <div class="col-md-3">
                                ...
                                <textarea class="form-control medical-history-field" id="medicalConditions" name="medical_conditions" placeholder="{{ T::translate('List all medical conditions', 'Ilagay ang lahat ng medikal na kondisyon')}}" rows="3">{{ old('medical_conditions') }}</textarea>
                                <small class="text-muted">{{ T::translate('Separate multiple conditions with commas', 'Paghiwalayin ang maraming kondisyon gamit ang kuwit')}}</small>
                            </div>
                            <div class="col-md-3">
                                <label for="medications" class="form-label">{{ T::translate('Medications', 'Mga Gamot')}}</label>
                                <textarea class="form-control medical-history-field" id="medications" name="medications" placeholder="{{ T::translate('List all medications', 'Ilagay ang lahat ng gamot')}}" rows="3">{{ old('medications') }}</textarea>
                                <small class="text-muted">{{ T::translate('Separate multiple medications with commas', 'Paghiwalayin ang maraming gamot gamit ang kuwit')}}</small>
                            </div>
                            <div class="col-md-3">
                                <label for="allergies" class="form-label">{{ T::translate('Allergies', 'Alerhiya')}}</label>
                                <textarea class="form-control medical-history-field" id="allergies" name="allergies" placeholder="{{ T::translate('List all allergies', 'Ilagay ang lahat ng alerhiya')}}" rows="3">{{ old('allergies') }}</textarea>
                                <small class="text-muted">{{ T::translate('Separate multiple allergies with commas', 'Paghiwalayin ang maraming alerhiya gamit ang kuwit')}}</small>
                            </div>
                            <div class="col-md-3">
                                <label for="immunizations" class="form-label">{{ T::translate('Immunizations', 'Mga Bakuna')}}</label>
                                <textarea class="form-control medical-history-field" id="immunizations" name="immunizations" placeholder="{{ T::translate('List all immunizations', 'Ilagay ang lahat ng bakuna')}}" rows="3">{{ old('immunizations') }}</textarea>
                                <small class="text-muted">{{ T::translate('Separate multiple immunizations with commas', 'Paghiwalayin ang maraming bakuna gamit ang kuwit')}}</small>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <!-- Category Dropdown -->
                        ...
                        <div class="row mb-2 align-items-center">
                            <div class="col-md-4">
                                <label class="form-label">{{ T::translate('Disease / Therapy Handling', 'Sakit / Pamamahala ng Therapy')}}</label>
                            </div>
                            <div class="col-md-4">
                       ...
                       <!-- Medication Management -->
                        ...
                        <div id="medicationManagement">
                            <div class="row mb-1 align-items-center medication-row">
                                ...
                                <div class="col-md-4">
                                    <textarea class="form-control" name="administration_instructions[]" placeholder="{{ T::translate('Enter Administration Instructions', 'Ilagay ang mga tagubilin sa pangangasiwa')}}" rows="1" ></textarea>
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="mb-1">
                                    ...
                                    <textarea class="form-control" id="walkingAbility" name="mobility[walking_ability]" 
                                            ...
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('mobility.walking_ability') }}</textarea>
                                </div>
                                <div class="mb-1">
                                    ...
                                    <textarea class="form-control" id="assistiveDevices" name="mobility[assistive_devices]" 
                                            placeholder="{{ T::translate('Enter details about assistive devices', 'Ilagay ang mga detalye sa kagamitang pantulong')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('mobility.assistive_devices') }}</textarea>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="transportationNeeds" name="mobility[transportation_needs]" 
                                            ...
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('mobility.transportation_needs') }}</textarea>
                                </div>
                            </div>

                            <!-- Cognitive Function Section -->
                            <div class="col-md-4">
                                <h5 class="text-start">{{ T::translate('Cognitive Function', 'Pag-andar ng Kognitibo')}}</h5>
                                <div class="mb-1">
                                    ...
                                    <textarea class="form-control" id="memory" name="cognitive[memory]" 
                                            placeholder="{{ T::translate('Enter details about memory', 'Ilagay ang mga detalye sa memorya')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('cognitive.memory') }}</textarea>
                                </div>
                                <div class="mb-1">
                                    ...
                                    <textarea class="form-control" id="thinkingSkills" name="cognitive[thinking_skills]" 
                                            placeholder="{{ T::translate('Enter details about thinking skills', 'Ilagay ang mga detalye sa kasanayan sa pag-iisip')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('cognitive.thinking_skills') }}</textarea>
                                </div>
                                <div class="mb-1">
                                    ...
                                    <textarea class="form-control" id="orientation" name="cognitive[orientation]" 
                                            placeholder="{{ T::translate('Enter details about orientation', 'Ilagay ang mga detalye sa oryentasyon')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('cognitive.orientation') }}</textarea>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="behavior" name="cognitive[behavior]" 
                                            placeholder="{{ T::translate('Enter details about behavior', 'Ilagay ang mga detalye sa pag-uugali')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('cognitive.behavior') }}</textarea>
                                </div>
                            </div>

                            <!-- Emotional Well-being Section -->
                            <div class="col-md-4">
                                <h5 class="text-start">{{ T::translate('Emotional Well-being', 'Kagalingang Emosyonal')}}</h5>
                                <div class="mb-1">
                                    <label for="mood" class="form-label">{{ T::translate('Mood', 'Kalooban')}}</label>
                                    <textarea class="form-control" id="mood" name="emotional[mood]" 
                                            placeholder="{{ T::translate('Enter details about mood', 'Ilagay ang mga detalye sa kalooban')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('emotional.mood') }}</textarea>
                                </div>
                                <div class="mb-1">
                                    ...
                                    <textarea class="form-control" id="socialInteractions" name="emotional[social_interactions]" 
                                            placeholder="{{ T::translate('Enter details about social interactions', 'Ilagay ang mga detalye sa pakikipag-ugnayan sa lipunan')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('emotional.social_interactions') }}</textarea>
                                </div>
                                <div class="mb-3">
                                    <label for="emotionalSupport" class="form-label">{{ T::translate('Emotional Support Need', 'Pangangailangan ng Suportang Emosyonal')}}</label>
                                    <textarea class="form-control" id="emotionalSupport" name="emotional[emotional_support]" 
                                            placeholder="{{ T::translate('Enter details about emotional support need', 'Ilagay ang mga detalye sa pangangailangan ng suportang emosyonal')}}" rows="2" 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, and parentheses are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, at panaklong lamang ang pinapayagan.')}}">{{ old('emotional.emotional_support') }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-1">
                            <div class="col-12">
                                <h5 class="text-start">{{ T::translate('Emergency Contact', 'Kontak sa Emergency')}}</h5>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <!-- Contact Name -->
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control" id="contactName" name="emergency_contact[name]" 
                                    ...
                                    placeholder="{{ T::translate('Enter contact name', 'Ilagay ang pangalan ng kontak')}}" 
                                    required >
                            </div>
                            ...
                            <!-- Mobile Number -->
                            <div class="col-md-3">
                                <label for="emergencyMobileNumber" class="form-label">{{ T::translate('Mobile Number', 'Numero ng Mobile')}}<label style="color:red;"> * </label></label>
                                <div class="input-group">
                                    <span class="input-group-text">+63</span>
                                    <input type="text" class="form-control" id="emergencyMobileNumber" name="emergency_contact[mobile]" 
                                        ...
                                        title="{{ T::translate('Must be 10 digits.', 'Dapat 10 digits.')}}">
                                </div>
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="emergencyProcedures" name="emergency_plan[procedures]" 
                                        ...
                                        required 
                                        pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                        title="{{ T::translate('Only letters, numbers, spaces, commas, periods, hyphens, parentheses, single quotes, double quotes, apostrophes, and exclamation/question marks are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, gitlang, panaklong, solong panipi, dobleng panipi, apostrope, at tandang pananong/padamdam lamang ang pinapayagan.')}}">{{ old('emergency_plan.procedures') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="row mb-1">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select" id="careworkerName" name="care_worker[careworker_id]" required>
                                    <option value="" disabled {{ old('care_worker.careworker_id') ? '' : 'selected' }}>{{ T::translate('Select Care Worker', 'Pumili ng Tagapag-alaga')}}<label style="color:red;"> * </label></option>
                                ...
                                <div id="tasksContainer">
                                    <div class="input-group mb-2">
                                        <input type="text" class="form-control" name="care_worker[tasks][]"
                                            ...
                                            required 
                                            pattern="^[A-Za-z0-9\s.,\-()'\"+’!?]+$"!?]+$" 
                                            title="{{ T::translate('Only letters, numbers, spaces, commas, periods, and hyphens are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, at gitlang lamang ang pinapayagan.')}}">
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Beneficiary Picture -->
                            <div class="col-md-3">
                                ...
                                <input type="file" class="form-control" id="beneficiaryProfilePic" name="beneficiaryProfilePic" 
                                    ...
                                    title="{{ T::translate('Only PNG and JPEG images are allowed.', 'Tanging PNG at JPEG na larawan lamang ang pinapayagan.')}}">
                                @if($errors->any())
                                <small class="text-danger">{{ T::translate('Note: You need to select the file again after a validation error.', 'Tandaan: Kailangang pumili muli ng file pagkatapos ng validation error.')}}</small>
                                @endif
                                <small class="text-danger">{{ T::translate('Maximum file size: 7MB', 'Pinakamalaking laki ng file: 7MB')}}</small>
                            </div>
                            ...
                            <div class="col-md-3">
                                <label for="careServiceAgreement" class="form-label">{{ T::translate('Care Service Agreement', 'Kasunduan sa Serbisyo ng Pag-aalaga')}}</label>
                                <input type="file" class="form-control" id="careServiceAgreement" name="care_service_agreement" 
                                    accept=".pdf,.doc,.docx" 
                                    required 
                                    title="{{ T::translate('Only PDF, DOC, and DOCX files are allowed.', 'Tanging PDF, DOC, at DOCX na file lamang ang pinapayagan.')}}">
                                @if($errors->any())
                                <small class="text-danger">{{ T::translate('Note: You need to select the file again after a validation error.', 'Tandaan: Kailangang pumili muli ng file pagkatapos ng validation error.')}}</small>
                                @endif
                                <small class="text-danger">{{ T::translate('Maximum file size: 5MB', 'Pinakamalaking laki ng file: 5MB')}}</small>
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                @if($errors->any())
                                <small class="text-danger">{{ T::translate('Note: You need to upload/sign again after a validation error.', 'Tandaan: Kailangang mag-upload/pumirma muli pagkatapos ng validation error.')}}</small>
                                @endif
                                </div>
                                <div class="row mb-1">
                                    <div class="col-md-12">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="row mb-1">
                                    <div class="col-md-12">
                                        <label for="careWorkerSignatureUpload" class="form-label">{{ T::translate('Upload Care Worker Signature', 'Mag-upload ng Lagda ng Tagapag-alaga')}}</label>
                        ...
                        <div class="row mb-1">
                            <div class="col-12">
                                <h5 class="text-start">{{ T::translate('Login Access', 'Access sa Pag-login')}}</h5> 
                            </div>
                        </div>
                        <div class="row mb-3">
                            <!-- Email -->
                            <div class="col-md-4">
                                <label for="generatedUsername" class="form-label">{{ T::translate('Username (Auto-generated)', 'Username (Awtomatikong nabuo)')}}</label>
                                <div class="input-group">
                                    <input type="text" class="form-control" id="generatedUsername" readonly 
                                        value="{{ T::translate('Username will be generated from name fields', 'Ang username ay mabubuo mula sa mga field ng pangalan')}}" disabled>
                                    <span class="input-group-text"><i class="bi bi-info-circle" title="{{ T::translate('Username is automatically generated from your name: first initial + middle initial + last name', 'Ang username ay awtomatikong nabubuo mula sa iyong pangalan: unang inisyal + gitnang inisyal + apelyido')}}"></i></span>
                                </div>
                                <small class="text-muted">{{ T::translate('The system will create a username based on the beneficiary\'s name.', 'Ang system ay gagawa ng username batay sa pangalan ng benepisyaryo.')}}</small>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    <input type="password" class="form-control" id="confirmPassword" name="account[password_confirmation]" placeholder="{{ T::translate('Confirm password', 'Kumpirmahin ang password')}}" required title="{{ T::translate('Passwords must match.', 'Dapat magtugma ang mga password.')}}">
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-footer justify-content-center">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">{{ T::translate('OK', 'Sige')}}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="fileSizeErrorModalLabel">{{ T::translate('File Size Error', 'Error sa Laki ng File')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <script>
        ...
        newRow.innerHTML = `
            ...
            <div class="col-md-4">
                <textarea class="form-control" name="administration_instructions[]" placeholder="{{ T::translate('Enter administration instructions', 'Ilagay ang mga tagubilin sa pangangasiwa')}}" rows="1">${instructions}</textarea>
            </div>
            <div class="col-md-1 d-flex text-start">
                <button type="button" class="btn btn-danger" onclick="removeMedicationRow(this)">{{ T::translate('Delete', 'Tanggalin')}}</button>
            </div>
    ...
    <script>
        ...
        function addTask(taskValue = '') {
            ...
            input.title = "{{ T::translate('Only letters, numbers, spaces, commas, periods, and hyphens are allowed.', 'Tanging mga letra, numero, espasyo, kuwit, tuldok, at gitlang lamang ang pinapayagan.')}}";
            ...
            if (input.files.length > 0) {
                ...
                if (file.size > maxSize) {
                    ...
                    fileSizeErrorMessage.innerHTML = `
                        <strong>{{ T::translate('Form submission failed', 'Nabigo ang pagsusumite ng form')}}</strong><br>
                        ${input.previousElementSibling.textContent} (${fileSizeMB}MB) {{ T::translate('exceeds the maximum size of', 'lumampas sa maximum na laki ng')}} ${maxSizeMB}MB.<br>
                        {{ T::translate('Please select a smaller file or compress your existing file.', 'Mangyaring pumili ng mas maliit na file o i-compress ang iyong kasalukuyang file.')}}
                    `;
   ...
   <script>
    ...
    function updateBarangays() {
        ...
        barangayDropdown.innerHTML = '<option value="" disabled selected>{{ T::translate('Select barangay', 'Pumili ng Barangay')}}</option>';
        ...
        if (!found) {
            const option = document.createElement('option');
            option.value = "";
            option.textContent = "{{ T::translate('No barangays available for this municipality', 'Walang available na barangay para sa munisipalidad na ito')}}";
...
function geocodeAddress() {
    ...
    geocoder.geocode({ 'address': address }, function(results, status) {
        if (status === 'OK') {
            ...
            alert('{{ T::translate('Geocode was not successful for the following reason:', 'Ang Geocode ay hindi matagumpay dahil sa sumusunod na dahilan:')}} ' + status);
        }
    });
}
</script>