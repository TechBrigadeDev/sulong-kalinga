...
<head>
    ...
    <title>Emergency & Request History | Care Worker</title>
...
<body>
    ...
    <div class="home-section">
        <div class="page-header mb-2">
            ...
            <button class="history-btn active" onclick="window.location.href='{{ route('care-worker.emergency.request.index') }}'">
    ...
    <script>
        ...
        $('#applyFilter').on('click', function() {
            ...
            if (timeRange === 'custom') {
                ...
                if (new Date(startDate) > new Date(endDate)) {
                    toastr.error('{{ T::translate("Start date cannot be after end date", "Ang petsa ng simula ay hindi maaaring pagkatapos ng petsa ng pagtatapos")}}');
                    return;
                }
            }
            ...
            // Send filter request
            $.ajax({
                url: "{{ route('care-worker.emergency.request.filter.history') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        toastr.success('{{ T::translate("Date range filter applied successfully", "Matagumpay na na-apply ang pagsala ng petsa")}}');
                    } else {
                        toastr.error('{{ T::translate("Failed to apply filter", "Nabigong mag-apply ng pagsala")}}');
                    }
                },
                error: function(xhr) {
                    let errorMessage = '{{ T::translate("Failed to apply filter", "Nabigong mag-apply ng pagsala")}}';
                ...
                complete: function() {
                    $('#applyFilter').html('<i class="bi bi-funnel-fill me-1"></i>{{ T::translate("Apply Filter","I-Apply ang Pagsala")}}');
        ...
        function viewServiceRequestHistory(requestId) {
            ...
            $.ajax({
                ...
                error: function() {
                    $('#serviceHistoryContent').html('<div class="alert alert-danger">{{ T::translate("Failed to load service request details. Please try again.", "Nabigong i-load ang mga detalye ng pakiusap na serbisyo. Mangyaring subukan muli.")}}</div>');
                }
            });
        }
        ...
        function renderEmergencyHistoryDetails(emergency) {
            let content = `
                <div class="mb-4">
                    ...
                    <div class="row mb-2">
                        <div class="col-md-4 fw-bold">{{ T::translate("Emergency Contact:", "Emergency na Kontak:")}}</div>
                    ...
                    <div class="row mb-2">
                        <div class="col-md-4 fw-bold">{{ T::translate("Created:", "Nagawa:")}}</div>
            ...
            if (emergency.updates && emergency.updates.length > 0) {
                ...
                $('#emergencyHistoryTimeline').html('<p class="text-muted">{{ T::translate("No updates available", "Walang available na mga update")}}</p>');
            }
        }
        ...
        function renderServiceHistoryDetails(request) {
            let content = `
                <div class="mb-4">
                    ...
                    <div class="row mb-2">
                        <div class="col-md-4 fw-bold">{{ T::translate("Message:", "Mensahe:")}}</div>
                        <div class="col-md-8">${request.message || '{{ T::translate("No message provided", "Walang ibinigay na mensahe")}}'}</div>
                    </div>
                </div>
            `;
            ...
            if (request.updates && request.updates.length > 0) {
                ...
                $('#serviceHistoryTimeline').html('<p class="text-muted">{{ T::translate("No updates available", "Walang available na mga update")}}</p>');
            }
        }
            ...
            return `
                <div class="card emergency-card mb-3">
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap mb-1">
                            <span class="info-label">{{ T::translate("Message:", "Mensahe:")}}</span>
                            <span>${emergency.message ? (emergency.message.length > 100 ? emergency.message.substring(0, 100) + '...' : emergency.message) : '{{ T::translate("No message provided", "Walang ibinigay na mensahe")}}'}</span>
                        </div>
       ...
       function renderServiceHistoryCard(request) {
        ...
        function formatTimeAgo(dateTimeStr) {
            ...
            if (diffDays > 30) {
                ...
                return `${diffDays} {{ T::translate("day", "araw")}}${diffDays > 1 ? 's' : ''} {{ T::translate("ago", "ang nakalipas")}}`;
            } else if (diffHours > 0) {
                return `${diffHours} {{ T::translate("hour", "oras")}}${diffHours > 1 ? 's' : ''} {{ T::translate("ago", "ang nakalipas")}}`;
            } else {
                return '{{ T::translate("Less than an hour ago", "Mas mababa sa isang oras ang nakalipas")}}';
            }
        }
