...
class DatabaseSeeder extends Seeder
{
                ...
                if ($visitations->isEmpty()) {
                    ...
                    while ($currentDate->lte($endDate)) {
                        ...
                        foreach ($days as $day) {
                            ...
                            $weeklyCarePlan->assessment = $this->getRandomAssessment($beneficiary->gender);
                            $weeklyCarePlan->evaluation_recommendations = $this->getRandomEvaluation($beneficiary->gender);
     ...
     */
    private function getRandomAssessment($gender = null)
    {
        $pairs = $this->getAllAssessmentEvaluationPairs();
        if ($gender) {
            $pairs = array_filter($pairs, function($pair) use ($gender) {
                return !isset($pair['gender']) || strtolower($pair['gender']) === strtolower($gender);
            });
        }
     ...
     */
    private function getRandomEvaluation($gender = null)
    {
        $pairs = $this->getAllAssessmentEvaluationPairs();
        if ($gender) {
            $pairs = array_filter($pairs, function($pair) use ($gender) {
