...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // 12. Generate Tagalog NLP test data
        $this->generateTagalogNlpTestData();
    }
    ...
    /**
     * Generate Tagalog NLP test data
     */
    private function generateTagalogNlpTestData()
    {
        \Log::info("Generating Tagalog NLP test data...");
        
        // Get care worker ID to use consistently across all records
        ...
        $vitalSignsIds = $this->createTagalogTestVitalSigns($careWorkerId);

        // Insert weekly care plans with Tagalog assessments and evaluations
        $this->insertTagalogTestWeeklyCarePlans($vitalSignsIds, $careWorkerId, $beneficiaryIds);
        
        \Log::info("Tagalog NLP test data generation complete");
    }

    /**
     * Create vital signs records for the Tagalog NLP test data
     * 
     * @param int $careWorkerId - The care worker ID to use as created_by
     * @return array - List of created vital signs IDs
     */
    private function createTagalogTestVitalSigns($careWorkerId)
    {
        $vitalSignsData = [
            [
    ...
    /**
     * Insert weekly care plans with Tagalog test data
     * 
     * @param array $vitalSignsIds - The vital signs IDs to link to care plans
     * @param int $careWorkerId - The care worker ID to use
     * @param array $beneficiaryIds - Beneficiary IDs to use
     */
    private function insertTagalogTestWeeklyCarePlans($vitalSignsIds, $careWorkerId, $beneficiaryIds)
    {
